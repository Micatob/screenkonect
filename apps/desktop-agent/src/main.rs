...
async fn main() -> Result<()> {
    ...
    consent::wait_for_consent(&config).await?;
    ...
    let signaling_client = signaling::SignalingClient::connect(&config).await?;

    let webrtc_peer = webrtc::WebRtcPeer::new(&config, signaling_client).await?;
